...
def main():
	...
	#-------------------------------------------------------------------------------------------------------------------------
    	...
    	child.setecho(False)
    	child.expect(CUDA_GDB_EXPECT)  #"(cuda-gdb)"
    	child.sendline(BREAKPOINT+" "+BREAK_LOCATION) #break matrixMul.cu:63
    	child.expect(CUDA_GDB_EXPECT)#(cuda-gdb)
	wc = child.sendline(RUN)#run
    	resend = child.expect([CUDA_GDB_EXPECT,THREAD_CONTINUE_EXPECT])#(cuda-gdb) ---Type \<return\> to continue, or q \<return\> to quit---
    	if resend == 1: # type return
       		child.sendline()
    	rawstr = child.before# break matrixMul.cu:63
	lines = rawstr.split("\r\n")
